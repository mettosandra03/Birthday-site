...
// ---- 1. Load the saved card data ----
const saved = JSON.parse(localStorage.getItem('birthdayCard') || '{}');
const name = saved.name || '';
const message = saved.message || 'Hope your day is amazing!';
...
// Match the font/background the person picked on page 1, so both pages feel like one card.
if (saved.font) {
  document.querySelectorAll('.candle-heading, .after-message h2').forEach((el) => {
    el.classList.add(saved.font);
  });
}
...
if (saved.bg) {
  const color = saved.bg.type === 'color' ? saved.bg.value : presetGradients[saved.bg.value];
  if (color) {
